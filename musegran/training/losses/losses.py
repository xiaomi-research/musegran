"""Loss modules for diffusion and autoencoder training."""
...
class LossModule(nn.Module):
    """Base class for named, weighted loss modules."""
...
class ValueLoss(LossModule):
    """Pass-through loss: reads a pre-computed scalar from the info dict."""
...
class L1Loss(LossModule):
    """L1 loss between two tensors in the info dict, with optional mask."""
...
class MSELoss(LossModule):
    """MSE loss between two tensors in the info dict, with optional mask."""
...
class AuralossLoss(LossModule):
    """Wrapper for auraloss spectral losses (e.g., multi-resolution STFT)."""
...
class MultiLoss(nn.Module):
    """Aggregate multiple LossModule instances into a single total loss."""
